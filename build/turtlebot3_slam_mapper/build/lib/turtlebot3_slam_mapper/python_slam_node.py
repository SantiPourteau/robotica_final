...
class PythonSlamNode(Node):
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        delta_trans = 0.0  
        delta_rot2 = 0.0
        alpha1, alpha2, alpha3, alpha4 = 0.005, 0.005, 0.01, 0.005
        ...
        if self.prev_odom is not None:
            ...
            # Only update particles if movement exceeds 1cm threshold
            if delta_trans > 0.01 or abs(delta_theta) > 0.01:  # 1cm = 0.01m, 0.01 rad ≈ 0.57 degrees
    ...
    def resample_particles(self, particles):
        ...
        # Verificar si hay diversidad suficiente
        max_weight = max(weights)
        effective_particles = sum(1 for w in weights if w > max_weight * 0.1)
        
        # Si hay poca diversidad, añadir partículas aleatorias
        if effective_particles < len(particles) * 0.3:
            self.add_random_particles(particles, 0.2)  # 20% partículas aleatorias
    ...
    def add_random_particles(self, particles, fraction):
        ...
        for i in range(num_random):
            if i < len(particles):
                # Partícula aleatoria cerca de la mejor
