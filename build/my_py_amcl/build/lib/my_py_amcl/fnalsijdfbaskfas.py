...
class AmclNode(Node):
    ...
    def get_lookahead_point(self, current_pose):
        """ Get the next point in the path that is beyond the lookahead distance from the current pose. """
        ...
        # Buscar un punto más adelante en el path, pero no demasiado lejos
        # Usar una distancia fija en lugar de un índice fijo
        target_distance = 0.2  # 50cm de lookahead
        ...
        for i in range(closest_idx, len(self.current_path.poses)):
            ...
            if current_distance >= target_distance:
                # Check if this point is actually ahead of the robot
                dx = curr_pose.position.x - current_pose.position.x
                dy = curr_pose.position.y - current_pose.position.y
                angle_to_point = np.arctan2(dy, dx)
                angle_diff = abs(self.angle_diff(angle_to_point, robot_yaw))
                
                # Only use this point if it's within 90 degrees of robot's forward direction
                if angle_diff < np.pi/2:
                    return curr_pose
                else:
                    # If this point is behind the robot, continue searching
                    continue
        
        # If no suitable point found, use the last point if it's ahead
        last_pose = self.current_path.poses[-1].pose
        dx = last_pose.position.x - current_pose.position.x
        dy = last_pose.position.y - current_pose.position.y
        angle_to_last = np.arctan2(dy, dx)
        angle_diff = abs(self.angle_diff(angle_to_last, robot_yaw))
        
        if angle_diff < np.pi/2:
            return last_pose
        else:
            # If even the last point is behind, return the closest point ahead
            for i in range(closest_idx, len(self.current_path.poses)):
            ...
            # If no point ahead found, return the closest point
            return self.current_path.poses[closest_idx].pose
    
    def compute_control(self, current_pose, target_pose):
        """ Compute the control command to move towards the target pose. """
        ...
        angle_error = self.angle_diff(angle_to_target, yaw)
        
        if abs(angle_error) < np.pi/6:  # ~ 17 grados
            # Use even smaller gains for near-straight paths
            angular_gain = 0.075
        elif abs(angle_error) < np.pi/4: # ~ 22.5 grados
            angular_gain = 0.125
        else:
            angular_gain = 0.2

        # Debug logging for straight line issues
        if abs(angle_error) > 0.1:  # Only log when there's significant error
            self.get_logger().info(f"Angle debug - Target: {np.degrees(angle_to_target):.1f}°, Current: {np.degrees(yaw):.1f}°, Error: {np.degrees(angle_error):.1f}°")
            self.get_logger().info(f"Position - Current: ({current_pose.position.x:.2f}, {current_pose.position.y:.2f}), Target: ({target_pose.position.x:.2f}, {target_pose.position.y:.2f})")
        
        # # Deadzone para evitar oscilaciones pequeñas
        if abs(angle_error) < np.pi/24: # Deadzone de 1.5 grados
            angle_error = 0.0
        
        cmd = Twist()
        
        # Si el error de ángulo es grande, girar primero
        if abs(angle_error) > np.pi/16:
            cmd.linear.x = 0.0
            angular_cmd = angular_gain * angle_error
        else:
            cmd.linear.x = self.linear_velocity
            angular_cmd = angular_gain * angle_error  # Ganancia adaptativa
        
        # Aplicar suavizado temporal al comando angular
        # smoothed_angular = (self.angular_filter_alpha * angular_cmd + 
        #                    (1 - self.angular_filter_alpha) * self.last_angular_cmd)
        
        # cmd.angular.z = smoothed_angular
        # self.last_angular_cmd = smoothed_angular
        
        cmd.angular.z = angular_cmd
        self.last_angular_cmd = angular_cmd
        # Log velocity commands
        # self.get_logger().info(f'Velocity commands - Linear: {cmd.linear.x:.3f} m/s, Angular: {smoothed_angular:.3f} rad/s')
        self.get_logger().info(f'Velocity commands - Linear: {cmd.linear.x:.3f} m/s, Angular: {angular_cmd:.3f} rad/s')
